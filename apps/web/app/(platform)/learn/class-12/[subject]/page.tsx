/**
 * @file app/(platform)/learn/class-12/[subject]/page.tsx
 * @description Class 12 subject detail page — shows all chapters for a subject
 * Route: /learn/class-12/[subject]  (e.g. /learn/class-12/physics)
 * Mirrors the Class 9/10/11 subject page pattern for consistency across the platform.
 */
...
function getSubjectData(slug: string) {
  return CLASS_12_SUBJECT_DATA[slug] ?? {
    name:          slug.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()),
    description:   `Complete NCERT Class 12 ${slug} curriculum with structured lessons and board-exam practice.`,
    totalChapters: 10,
    duration:      "30 days",
    hasSimulation: false,
    isBoard:       true,
...
export async function generateStaticParams() {
  return Object.keys(CLASS_12_SUBJECT_DATA).map((subject) => ({ subject }));
}
...
export async function generateMetadata({
  ...
  return {
    title:       `Class 12 ${data.name} — NCERT | LearnVeda`,
    description: data.description,
    keywords:    [`Class 12 ${data.name}`, `NCERT Class 12 ${data.name}`, "CBSE Class 12 Board Exam"],
  };
}

export default async function Class12SubjectPage({
  params,
}: {
  params: Promise<{ subject: string }>;
}) {
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
            <div>
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                <h1 className="text-2xl sm:text-3xl font-bold">Class 12 — {data.name}</h1>
                {data.isBoard && (
            ...
            <div className="flex gap-2 shrink-0">
              <Button asChild size="sm" variant="outline">
                <Link href={`/learn/class-12/${subject}/${data.chapters[0]?.id ?? "chapter-01"}`}>
      ...
      <div className="container px-4 py-8">
        ...
        <div className="space-y-3">
          {data.chapters.map((chapter, index) => (
            <Link
              key={chapter.id}
              href={`/learn/class-12/${subject}/${chapter.id}`}
        ...
        {data.isBoard && (
          <div className="mt-4 rounded-2xl border bg-red-500/5 border-red-500/20 p-5">
            <p className="text-sm text-foreground font-semibold mb-1">📋 Board Exam Preparation</p>
            <p className="text-sm text-muted-foreground">
              This subject is part of the CBSE Board exam. Use the{" "}
              <Link href="/test-center" className="text-brand-500 hover:underline">full-length mock tests</Link>{" "}
              and <Link href="/ai-tutor" className="text-brand-500 hover:underline">AI Tutor</Link> for last-minute revision.
            </p>
          </div>
        )}
      </div>
