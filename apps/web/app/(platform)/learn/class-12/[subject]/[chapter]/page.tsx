/**
 * @file app/(platform)/learn/class-12/[subject]/[chapter]/page.tsx
 * @description Individual chapter learning page for Class 12
 * Route: /learn/class-12/[subject]/[chapter]
...
import { CLASS_12_SUBJECT_DATA } from "../../subject-data";

function getChapterContext(subject: string, chapterId: string) {
  const subjectData = CLASS_12_SUBJECT_DATA[subject];
...
export async function generateMetadata({
  ...
  return {
    title:       `${title} — Class 12 ${subject.replace(/-/g," ")} | LearnVeda`,
    description: `Study ${title} for CBSE Class 12 ${ctx?.subjectName ?? subject} with topic breakdown and board exam practice questions.`,
    keywords:    [`Class 12 ${title}`, `NCERT ${title}`, "CBSE Class 12 Board Exam"],
  };
}

export default async function Class12ChapterPage({
  params,
}: {
  params: Promise<{ subject: string; chapter: string }>;
}) {
      ...
      <div className="border-b">
        <div className="container px-4 py-4">
          <nav className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
            ...
            <Link href="/learn/class-12"               className="hover:text-foreground transition-colors">Class 12</Link>
            <ChevronRight className="h-3 w-3" />
            <Link href={`/learn/class-12/${subject}`}  className="hover:text-foreground transition-colors capitalize">{subjectLabel}</Link>
            <ChevronRight className="h-3 w-3" />
          ...
          <div className="lg:col-span-2 space-y-8">
            <div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2 flex-wrap">
                ...
                {isBoard && (
                  <Badge className="text-[9px] py-0 bg-red-500/10 text-red-600 border-red-500/20">
                    Board Exam
                  </Badge>
                )}
                {hasSimulation && (
              ...
              <div className="flex items-start gap-2 rounded-xl border border-primary/20 bg-primary/5 p-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Master {title} for CBSE Class 12 {ctx?.subjectName ?? subjectLabel} — covering {topics.join(", ")}.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="rounded-xl border bg-card p-4">
                <p className="text-sm text-muted-foreground mb-3">
                  Test your understanding of {title} with chapter-wise MCQs, previous year board questions, and mock tests.
                </p>
          ...
          <div className="space-y-4">
            ...
            {(ctx?.prevChapter || ctx?.nextChapter) && (
              <div className="rounded-2xl border bg-card p-5 space-y-2">
                {ctx?.prevChapter && (
                  <Link href={`/learn/class-12/${subject}/${ctx.prevChapter.id}`} className="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground transition-colors">
                    ← {ctx.prevChapter.title}
                  </Link>
                )}
                {ctx?.nextChapter && (
                  <Link href={`/learn/class-12/${subject}/${ctx.nextChapter.id}`} className="flex items-center justify-end gap-2 text-xs font-medium text-primary hover:underline">
